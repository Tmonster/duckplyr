...
#'
table_to_duckplyr_df <- function(con, table_name) {
	# TODO: better comparison to make sure the connections are **Actually** the same.
	if (get_default_duckdb_connection()@driver@dbdir != con@driver@dbdir) {
		error("the passed connection is not the default duckplyr connection")
	}
